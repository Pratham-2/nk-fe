...
const AddDecoratorModal = (props) => {
    ...
    useEffect(() => {
        if (props.decoratorDetails) {
            const getImageUrl = async (path) => {
                const imageUrl = await GetImage(`decorator-images/${path}`);
                setVenueBlobs( d => [...d, imageUrl]);
            }
            for (const venueBlob of props.decoratorDetails.Images) {
    ...
    useEffect(() => {
        $('#add-decorator-modal').modal({ backdrop: 'static'});
        $('#add-decorator-modal').on('hidden.bs.modal',function() { props.onDismissModal(props.decoratorDetails ? null :  false ) })
        ...
        if (props.decoratorDetails) {
            if (deletedImages && deletedImages.length > 0) {
                for (const image of deletedImages) {
                    await DeleteImage(`decorator-images/${image}`)
                }
            }
            let fileList = [];
            for(const image of addedImages) {
                ...
                FileUploadHelper('decorator-images', fileFullName, image);
            ...
            dispatch(updateDecorator({ ID: props.decoratorDetails.ID, ...values, VendorID : currentUser.ID, Images : updatedImageList })) // Venue Action Creator
        }
        else {
            let fileList = [];
    ...
    const openImageModal = (e) => {
        if (venueBlobs && venueBlobs.length > 0) {
            setEditImageCropper({
                ...
                header      : 'Edit Decorator Image'
            });
        } else {
            setCropperData({
                imageRatio  : { width : 1000, height : 500},
                dataHandler : (val) =>  setVenueBlobs(val),
                header      : 'Decorator Image'
            });
        }
    }
    ...
    return (
        <>
            <div className="modal fade"  tabIndex="-1" id="add-decorator-modal"  style={{display:'block'}}>
                <div className="modal-dialog mw-900px">
                    <div className="modal-content">
                        <div className="modal-header py-2">
                            <h5 className="modal-title">{props.decoratorDetails ? 'Edit Decorator' : 'Add Decorator'}</h5>   
                        ...
                        <form id="add-decorator-form" onSubmit={formik.handleSubmit}>       
                            ...
                            <div className="modal-footer">
                                <div className="row">
                                    ...
                                    <div className="col">
                                        <button type="submit" id="add-decorator" className="btn btn-primary" onClick={formik.handleSubmit}>{props.decoratorDetails ? 'Update' : 'Add'}</button>
                                    </div>
                                </div>
                            </div>
                        </form>
