...
const BeauticianHost = () => {

    const serviceName = 'Beautician';
    const dispatch    = useDispatch();

    const currentUser    = useSelector(state => state.authReducer.currentUser);
    const beauticianList = useSelector( s => s.serviceReducer.serviceList);

    const [serviceList    ,      setserviceList]        = useState([]);
    const [showAddModal   ,      setShowModal]          = useState(false);
    const [selectedService,      setSelectedService]    = useState();
    const [editService    ,      setEditService]        = useState();
    const [filteredBeautician,   setFilteredBeautician] = useState([]);
    ...
    useEffect(() => {
        if(serviceList.length > 0)
            ...
            setFilteredBeautician([])
    }, [serviceList, searchString]);

    const filterBeautician = async () => {

        if (searchString === '') {
            setFilteredBeautician(serviceList);
        } else {
            const matchedBeauticians = serviceList.filter((v) => {
    ...
    useEffect( () => {   
        if(beauticianList.length > 0){
            setserviceList(beauticianList);
        }
    }, [beauticianList])
        ...
        <>
                ...
                <div className="card-header">
                    <ul className="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                        <li className="nav-item mt-2">
                            <NavLink className="nav-link text-active-primary ms-0 me-10 py-5" to="/host/beauticians/all">
            ...
            {(
                <div className="margin-l25 margin-r25 margin-t20 row padding-0">
                    { (filteredBeautician.length > 0 ) ? (
                        filteredBeautician.map((v, i) => (
                                                ...
                                                <div className='col-sm-4'>
                                                    ...
                                                    <div className="dropdown">
                                                        ...
                                                        <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                                            ...
                                                            <li><a className="dropdown-item" onClick={() => deleteBeautician(v)}> Delete </a></li>
                                                        </ul>
                                                    </div>
                                                    </span>
                                                </div> 
            ...
            {showAddModal && (
                <AddServiceModal
                    ...
                    AddService      = {postBeautician}
                    UpdateService   = {updateBeautician}
                />
            )}
